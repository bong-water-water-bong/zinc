...
**Done so far:** nothing yet (effort just spawned; branch = origin/main `3da8b4f1` + this effort file + runner).
**Exact next step:** Start increment 1, step 1a (below) — add a `dbg_cuda batch` harness entrypoint + a slot-based KV allocation, behind a new code path that does NOT touch the production single-sequence `decodeStep`. Build, smoke-run.
**Open risks / notes:** none logged yet.
