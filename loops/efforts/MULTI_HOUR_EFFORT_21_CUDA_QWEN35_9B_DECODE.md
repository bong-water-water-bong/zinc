...
> **Status:** **M2 forward CORRECT on the 4090** — `forward_cuda` is token-for-token identical to llama.cpp CUDA (9/9 diverse tokens, top-5 logits match). Decode perf not yet measured (**0 tok/s** — the autoregressive loop + async ring are next). Goal: beat llama.cpp CUDA decode (**98 t/s on the 4090**) on the 4090 + 5090.
