...
kernel void main0(
    ...
    if (seven_col_tail) {
        float4 acc1_tail = float4(0.0f);
        float3 acc2_tail = float3(0.0f);
        for (uint b = tid; b < nb; b += 32u) {
            device const uchar* block = src + b * bpb;
            ...
            acc1_tail += d * sum_qx0 + m * sum_x0;
            acc2_tail += d * sum_qx1 + m * sum_x1;
        }

        const float out0 = simd_sum(acc1_tail.x);
        const float out1 = simd_sum(acc1_tail.y);
        const float out2 = simd_sum(acc1_tail.z);
        const float out3 = simd_sum(acc1_tail.w);
        const float out4 = simd_sum(acc2_tail.x);
        const float out5 = simd_sum(acc2_tail.y);
        const float out6 = simd_sum(acc2_tail.z);
