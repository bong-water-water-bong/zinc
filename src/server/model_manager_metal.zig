...
pub const ModelManager = struct {
    ...
    pub fn collectCatalogView(self: *ModelManager, allocator: std.mem.Allocator, include_all: bool) !ModelCatalogView {
        ...
        for (catalog_mod.entries) |entry| {
            ...
            // model is demonstrably running, so hiding it from /v1/models
            // just confuses clients that want to query the active model by id.
...
test "collectCatalogView enables qwen36 A3B thinking toggle for raw matched path on Metal" {
    var fake = ModelManager{
        .allocator = std.testing.allocator,
        .device = undefined,
        .profile = catalog_mod.profileForMetal(),
